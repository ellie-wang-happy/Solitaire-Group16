...
class TalonPileView extends StackPane implements GameModelListener
{
...
private void buildLayout()
{
	...
	if(GameModel.instance().getStack(1).size()>0)
	{
		...
		{
            int y = 0 ;
            if(GameModel.instance().getStack(1).size()>=3)
			{
				if(GameModel.instance().getStack(1).size()>0)
				{
					for(int i = GameModel.instance().getStack(	1).size() - 3 ; i<  GameModel.instance().getStack(	1).size() ; i ++)
					{
						Card cardView =  GameModel.instance().getStack(	1).peek(i);
						final ImageView image = new ImageView(CardImages.getCard(cardView));
						image.setTranslateX(Y_OFFSET * y);
						y++;
						getChildren().add(image);
			...
			else if(GameModel.instance().getStack(1).size()==2)
			{
				for(int i = GameModel.instance().getStack(	1).size() - 2 ; i<  GameModel.instance().getStack(	1).size() ; i ++)
				{
					if(GameModel.instance().getStack(1).size()>0)
					{
						Card cardView =  GameModel.instance().getStack(	1).peek(i);
		...
		else
		{
			...
			setOnDragEntered(createDragEnteredHandler(image, cardView));// 当你拖动到目标控件的时候，会执行这个事件回调。
			setOnDragExited(createDragExitedHandler(image, cardView));//当你拖动移出目标控件的时候，执行这个操作。
			setOnDragDropped(createDragDroppedHandler(image, cardView));//当你拖动到目标并松开鼠标的时候，执行这个DragDropped事
			image.setOnDragDetected(createDragDetectedHandler(image, cardView));//当你从一个Node上进行拖动的时候，会检测到拖动操作，将会执行这个EventHandler。
